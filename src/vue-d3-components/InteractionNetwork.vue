...
export default {
  ...
  watch: {
    accessionId: function () {
      this.drawPlot();
    },
    sRelationTypeIds: function () {
      this.drawPlot();
    },
  },
  methods: {
    drawPlot: function () {
      var that = this;
      var AccessionId = this.accessionId;
      var sRelationTypeIds = this.sRelationTypeIds;

      if (AccessionId && sRelationTypeIds) {
        d3.select(this.$el).selectAll("svg").remove();
        var svg = d3.select(this.$el).append("svg");

        var parentWidth = svg.node().parentNode.clientWidth;
        var parentHeight = svg.node().parentNode.clientHeight * 4;
        ...
        that.prepareGraph(svg, AccessionId, sRelationTypeIds);
      }
    },
    prepareGraph: function prepareGraph(svg, AccessionId, sRelationTypeIds) {
      ...
      d3.json(
        this.endpoint
          .replace("__parameter1__", AccessionId)
          .replace("__parameter2__", "")
          .replace("__parameter3__", sRelationTypeIds)
          .replace("__parameter4__", 0)
          .replace("__parameter5__", 1)
          .replace("__parameter6__", "")
      ).then( graph => {
    ...
    direct2AccessionId: function direct2AccessionId (endPointAccession, protein_id) {
      d3.json(endPointAccession.replace('__parameter1__', protein_id))
          .then(oAccessionId => this.drawPlot(that.endpoint, oAccessionId.AccessionId))
    },
    ...
    createSelectableForceDirectedGraph:
       ...
       */
        ...
        function expandNode(d) {
          ...
          d3.json(
              endpoint
                  .replace('__parameter1__', d.AccessionId)
                  .replace(
                      '__parameter2__',
                      d.ResourceTypeId === 1 ? '' : d.ResourceTypeId
                  )
                  .replace('__parameter3__', that.sRelationTypeIds)
                  .replace('__parameter4__', d.Offset++)
                  .replace('__parameter5__', 0)
                  .replace('__parameter6__', sExistingNodes))
              .then((data) => {
                function onlyUnique (value, index, self) {
                  return self.indexOf(value) === index
                }
                ...
                that.busy = false
                that.getResourceTypes(data.nodes)
                updateLinks() // TODO ?
              })
        }
        ...
        function enterNodes(n) {
          ...
          nodeText
            ...
            .text(function (d) {
              ...
              return d.Label.slice(0, 12) + "(...)" + d.Label.slice(-3);
            });
          ...
          nodeCircle
            ...
            .on("mouseout", function (e, d) {
              ...
              .on("click", (e,d) => {
                ...
                if(selectedNodeIds) {
                  d3.json(that.endpointSuperNodeInfo +
                      '?accession_id=__parameter1__'.replace(
                          '__parameter1__', selectedNodeIds))
                      .then((data) => {
                        ...
                        data = data.reduce(function (prev, next) {
                          if (prev.length === 0) {
                            return prev.concat({
                              Label: next.GeneName,
                              UniProtName: next.AccessionNumber,
                              Children: [next],
                            })
                          }
                          if (prev[prev.length - 1].Label === next.GeneName) {
                            prev[prev.length - 1].Children =
                                prev[prev.length - 1].Children.concat(next)
                            if (next.ResourceTypeId === 1) {
                              prev[prev.length - 1].UniProtName =
                        ...
                        data.forEach(function (e) {
                          e.CurveFit = e.Children.some(function (c) {
                            return c.CurveFit
                          })
                          e.BC = e.Children.some(function (c) {
                            return c.BC
                          })
                          e.Expression = e.Children.some(function (c) {
                            return c.Expression
                          })
                        })

                        var asUniProtNameHeatmap = data
                            .filter(function (e) {
                              return e.Expression
                            })
                            .map(function (x) {
                        ...
                        data = {
                          nodes: data,
                          asUniProtNameCt: asUniProtNameCt,
                          asUniProtNameHeatmap: asUniProtNameHeatmap,
                          bEnableHeatmap:
                              asUniProtNameHeatmap.length === 0 ? false : true,
                          bEnableCt: asUniProtNameCt.length === 0 ? false : true,
                        }

                        that.$emit('NodeModel', data)
                        that.$emit('keyChange', {
                          iTabId: 1,
                        })
  ...
  mounted: function () {
    if (this.accessionId !== null && this.sRelationTypeIds !== null) {
      this.drawPlot();
    }
  },
};
