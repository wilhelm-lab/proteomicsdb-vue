...
export default {
  props: {
    ...
    selectedLineTitlePrefix: {
      type: String,
      default: ''
    },
    changeSignOfSelectedLine: {
      type: Boolean,
      default: false
    },
    nHistogramBins: {
  ...
  computed: {
    minValue() {
      return this.minLineValue ? this.minLineValue - 2 : Math.min(...this.fullChartData)
    },
    maxValue() {
      return this.maxLineValue ? this.maxLineValue + 2 : Math.max(...this.fullChartData)
    }
  },
  watch: {
  ...
  methods: {
    drawPlot: function (data) {
      ...
      var thresholds = x.ticks(this.nHistogramBins - 1);
      ...
      const nclass = x.ticks(this.nHistogramBins - 1).length;
    ...
    drawLines: function (oLines) {
      ...
      lines
          ...
          .style("stroke", "transparent")
          .on("mouseenter", (e, d) => {
            d3.select(this.$el).select(`#selectedLine-${d.curveid}`).classed("highlightLines", true)
          })
          .on("mouseout", (e, d) => {
            d3.select(this.$el).select(`#selectedLine-${d.curveid}`).classed("highlightLines", false)

          })
          .append("title").text(d => `${this.selectedLineTitlePrefix}${d.value.toPrecision(3) * (this.changeSignOfSelectedLine ? -1 : 1)}`)
