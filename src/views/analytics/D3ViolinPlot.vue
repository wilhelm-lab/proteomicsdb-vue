...
export default {
  ...
  computed: {
    ...
    labelOffsetStyles: function () {
      ...
      const margin = this.oChartObjects.margin
  ...
  methods: {
    ...
    drawChartWithData: function drawChartWithData () {
      ...
      const svgs = d3.select(this.$el).select('.sapProteomicsdbViolinPlot').selectAll('svg')
      svgs.remove()
      ...
      const svg = d3.select(this.$el).select('.sapProteomicsdbViolinPlot').append('svg:svg')
      ...
      for (const [i, oSortedElement] of Object.entries(this.oChartObjects.oSortedData)) {
        const g = svg.append('g').attr('transform', 'translate(' + (j * (plotWidth + plotSpacing) + margin.left) + ',0)')
